...
export const generateMultiAgentReport = (params = {}) => {
  return api.post('/train/report/multi-agent/generate', params).then(response => {
    if (response.data.code === 200) {
      return response
    } else {
      throw new Error(response.data.msg || '生成多智能体报告失败')
    }
  })
}
...
export const getMultiAgentReport = (params = {}) => {
  return api.get('/train/report/multi-agent', { params }).then(response => {
    if (response.data.code === 200) {
      return response
    } else {
      throw new Error(response.data.msg || '获取多智能体报告失败')
    }
  })
}

// 获取多智能体报告历史列表
export const getMultiAgentReportHistory = (params = {}) => {
  return api.get('/train/report/multi-agent/history', { params }).then(response => {
    if (response.data.code === 200) {
      return response
    } else {
      throw new Error(response.data.msg || '获取报告历史失败')
    }
  })
}

// 清除多智能体报告缓存
export const clearMultiAgentCache = () => {
  return api.post('/train/report/multi-agent/cache/clear').then(response => {
    if (response.data.code === 200) {
      return response
    } else {
...
// 检查多智能体服务健康状态
export const checkMultiAgentHealth = () => {
  return api.get('/train/report/multi-agent/health').then(response => {
    return response
  }).catch(() => {
    return { data: { code: 500, msg: '服务不可用' } }
  })
}
