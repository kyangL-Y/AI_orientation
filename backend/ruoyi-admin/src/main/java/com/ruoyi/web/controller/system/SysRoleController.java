...
public class SysRoleController extends BaseController
{
    ...
    public AjaxResult edit(@Validated @RequestBody SysRole role)
    {
        ...
        {
            forceLogoutRoleUsers(role.getRoleId());
            // 更新缓存用户权限
    ...
    public AjaxResult cancelAuthUser(@RequestBody SysUserRole userRole)
    {
        ...
        if (rows > 0)
        {
            tokenService.forceLogoutByUserId(userRole.getUserId());
        }
        return toAjax(rows);
    }
    ...
    private void forceLogoutRoleUsers(Long roleId)
    {
        if (roleId == null)
        {
            return;
        }
        Long[] userIds = roleService.selectUserIdsByRoleId(roleId);
        tokenService.forceLogoutByUserIds(userIds);
    }
}
