...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT 1',
  'ALTER TABLE learning_plans ADD COLUMN path_id bigint(20) DEFAULT NULL COMMENT ''所属学习路径ID'' AFTER plan_id, ADD INDEX idx_path_id (path_id)'
));
...
-- 插入示例数据
INSERT INTO `learning_paths` (`path_name`, `path_description`, `path_type`, `target_role`, `difficulty_level`, `estimated_duration`, `status`, `sort_order`) VALUES
('前台服务专家路径', '针对酒店前台岗位的完整培训路径，从基础到进阶全面提升', 'standard', '前台接待', 'intermediate', 90, 'active', 1),
('营销运营进阶路径', '酒店线上营销和运营管理的系统化培训路径', 'standard', '营销经理', 'advanced', 120, 'active', 2),
('新员工入职路径', '新员工快速熟悉酒店业务的标准化入职培训路径', 'standard', '全员', 'beginner', 30, 'active', 3);

INSERT INTO `learning_path_plans` (`path_id`, `plan_id`, `sequence`, `is_required`)
SELECT p.path_id, lp.plan_id, 99, 1
FROM learning_paths p
JOIN learning_plans lp ON lp.title = '学习路径结业考试'
WHERE p.path_name = '新员工入职路径';
