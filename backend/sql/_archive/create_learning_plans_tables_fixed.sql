...
('安全规范培训', '酒店安全操作规范培训', 1, 100, 'active', '2025-01-20', '2025-02-05', 'admin'),
('学习路径结业考试', '学习路径内的结业考试安排', 1, 2, 'active', '2025-02-01', '2025-02-10', 'admin');

-- 获取刚插入的计划ID
SET @plan1_id = LAST_INSERT_ID();
SET @plan2_id = @plan1_id + 1;
SET @plan3_id = @plan1_id + 2;
SET @graduation_exam_id = (SELECT id FROM train_exam WHERE name = '学习路径结业考试' ORDER BY id DESC LIMIT 1);

INSERT INTO `plan_items` (`plan_id`, `content_type`, `content_id`, `title`, `due_date`, `status`, `create_by`) VALUES
(@plan1_id, 'course', 101, '酒店文化介绍', '2025-01-18', 'pending', 'admin'),
(@plan1_id, 'quiz', 201, '入职考试', '2025-01-25', 'pending', 'admin'),
(@plan1_id, 'task', 301, '部门实习', '2025-01-30', 'pending', 'admin'),
(@plan2_id, 'course', 102, '消防安全知识', '2025-01-25', 'pending', 'admin'),
(@plan2_id, 'quiz', 202, '安全规范考试', '2025-02-05', 'pending', 'admin'),
(@plan3_id, 'quiz', IFNULL(@graduation_exam_id, 0), '结业考试', '2025-02-10', 'pending', 'admin');
