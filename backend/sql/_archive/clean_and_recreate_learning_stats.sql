...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     ...
     AND index_name = 'idx_answer_attempt_user_time') > 0,
    'DROP INDEX idx_answer_attempt_user_time ON train_answer_attempt',
    'SELECT "Index idx_answer_attempt_user_time does not exist"'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE table_schema = DATABASE() 
     AND table_name = 'train_answer_attempt' 
     AND index_name = 'idx_answer_attempt_user_correct') > 0,
    'DROP INDEX idx_answer_attempt_user_correct ON train_answer_attempt',
    'SELECT "Index idx_answer_attempt_user_correct does not exist"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE table_schema = DATABASE() 
     AND table_name = 'train_answer_attempt' 
     AND index_name = 'idx_answer_attempt_time') > 0,
    'DROP INDEX idx_answer_attempt_time ON train_answer_attempt',
    'SELECT "Index idx_answer_attempt_time does not exist"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT "Index idx_answer_attempt_user_time already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT "Index idx_answer_attempt_user_correct already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT "Index idx_answer_attempt_time already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 8. 测试查询
-- 获取用户学习统计概览
SELECT 'Testing comprehensive stats...' as status;
SELECT * FROM v_comprehensive_learning_stats WHERE user_id = 1 LIMIT 1;

-- 获取学习趋势数据
SELECT 'Testing learning trend...' as status;
CALL GetLearningTrend(1, 'daily');

-- 获取学习成就
SELECT 'Testing learning achievements...' as status;
CALL GetLearningAchievements(1);

-- 简化的趋势数据查询（替代函数）
SELECT 'Testing simplified trend query...' as status;
SELECT 
...
SELECT 'Learning statistics setup completed successfully!' as final_status;
