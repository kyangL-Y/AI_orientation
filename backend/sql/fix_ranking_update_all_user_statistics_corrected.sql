-- 修复排行榜：批量更新所有用户的统计数据（修正版）
-- 执行时间: 2026-02-05
-- 问题: user_statistics 表为空,导致排行榜不显示任何记录
-- 修正: 使用正确的字段名 attempt_time 而不是 submit_time
...
SELECT 
    ...
    MAX(attempt_time) as last_activity_time,
    NOW() as create_time,
    NOW() as update_time
FROM hotel_training.train_answer_attempt
WHERE attempt_time IS NOT NULL
GROUP BY user_id
...
SELECT 
    ...
    NOW() as update_time
FROM hotel_training.train_answer_attempt
WHERE attempt_time IS NOT NULL AND answer_type = 'exam'
GROUP BY user_id
ON DUPLICATE KEY UPDATE
    total_questions = VALUES(total_questions),
    correct_answers = VALUES(correct_answers),
...
SELECT 
    ...
    NOW() as update_time
FROM hotel_training.train_answer_attempt
WHERE attempt_time IS NOT NULL AND answer_type = 'practice'
GROUP BY user_id
ON DUPLICATE KEY UPDATE
    total_questions = VALUES(total_questions),
    correct_answers = VALUES(correct_answers),
